...
if( isset($_POST["id"]) && !empty($_POST["id"]) ){
	...
	for( $i = 0; $i < sizeof($servers); $i++ ){
		...
		if( !in_array(strtolower($domain),$notWanted) && isset($servers[$i]["id"]) ){
			$links .= "<div class='col-3 p-1'><a class='btn btn-secondary w-100' style='color:white' href='#' data-server='{$servers[$i]["i"]}' id='{$servers[$i]["id"]}' onclick='sendIdToIframe2(\"{$servers[$i]["id"]}\"); return false;'>Serv-{$y}</a></div>";
			$server = $servers[$i]["i"];
