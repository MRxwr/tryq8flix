...
function scrapEl7l($url) {
	...
	if ($dom) {
		if ($mainSection) {
			foreach ($mainSection->find('li') as $movie) {
				$link = $movie->find('.pm-video-thumb a', 0);
				$image = $movie->find('.pm-video-thumb img', 0);
				$title = $movie->find('.caption h3 a', 0);
				$duration = $movie->find('.pm-label-duration', 0);
				
				// Category might not be directly available in the new structure
				// We'll leave it empty or extract it from the title if needed
				$category = '';
				
				// Check if it's a TV show episode (modify this based on actual pattern)
				$episode = '';
				...
				$movieData = [
					...
					'category' => $category,
					'episode' => $episode,
					'description' => $duration ? $duration->plaintext : '',
				];
...
function extractSeasonUrlEl7l($html) {
...
function El7lListing($url) {
    ...
    if (strpos(strtolower($_POST["id"]), 'season') === false) {
        ...
        $html = extractSeasonUrlEl7l($html);
    }
    $html = curlCall($html);
