 <?php
	...
	if($apidata['token']){
		...
		if(isset($apidata['SiteUrl'])){ $CallBackUrl= $apidata['SiteUrl'].'/details'; }else{ $CallBackUrl =''; }
		if(isset($apidata['SiteUrl'])){ $ErrorUrl= $apidata['SiteUrl'].'/checkout.php?status=fail'; }else{ $ErrorUrl =''; }
		...
		if(isset($_POST['CustomerMobile'])){ $CustomerMobile = preg_replace('/\D/', '', $_POST['CustomerMobile']); }else{ 
			$PostalCode =''; 
	...
	// echo json_encode($data);
	// exit;
//.json_encode($CustomerAddress).',
	$postData='{
        "PaymentMethodId":"'.$PaymentMethodId.'",
        "CustomerName": "'.$CustomerName.'",
        "DisplayCurrencyIso": "'.$DisplayCurrencyIso.'",
        "MobileCountryCode":"'.$MobileCountryCode.'",
        "CustomerMobile": "'.substr($CustomerMobile, 0, 11).'",
