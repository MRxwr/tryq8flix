...
if( isset($_POST["type"]) && !empty($_POST["type"]) ){
    if( $_POST["type"] == "add" ){
        if( isset($_POST["id"]) && !empty($_POST["id"]) ){
            ...
            if ( selectDB("likedvidoes","`userId` = {$user["id"]} AND `postId` = {$_POST["id"]}") ){
                deleteDB("likedvidoes","`userId` = {$user["id"]} AND `postId` = {$_POST["id"]}");
                $msg = "Video has been removed from liked list";
                echo $msg; 
            }else{
                insertDB("likedvidoes",$data);
                echo 0;
            }
        }else{
        ...
        if( !empty($user["id"]) && $liked = selectDB("likedvidoes","`userId` = {$user["id"]}") ){
						...
						<div class='card w-100'>
							...
							<div style='height:200px; overflow:auto' >
								...
								<div class='row w-100 p-0 m-0'>
									...
									<div class='col-2 p-1'>
										<div class='btn btn-danger w-100 likedVidoes' id='{$post[0]["id"]}'><i class='bi bi-hand-thumbs-up-fill'></i></div>
									</div>
									<div class='col-2 p-1'>
										<div class='btn btn-secondary w-100 disLikedVidoes' id='{$post[0]["id"]}'><i class='bi bi-hand-thumbs-down-fill'></i></div>
									</div>
									<div class='col-2 p-1'>
