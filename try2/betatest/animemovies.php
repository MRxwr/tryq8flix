...
$sql = "SELECT language FROM category WHERE type like '%animov%'";
...
$sql = "SELECT genre FROM category WHERE type like '%animov%'";
$result = $dbconnect->query($sql);
$NumberOfPosts = $result->num_rows;
$genre = array();
$allgenres = array();
...
if ( isset( $_GET["sfc"] ) AND !isset( $_GET["sfg"] ) )
{
	...
	$sql = "SELECT * FROM category WHERE type LIKE '%animov%' AND language LIKE '%$lang%'";
	$result = $dbconnect->query($sql);
	$NumberOfPosts = $result->num_rows;
...
elseif ( !isset($_GET["sfc"]) AND isset($_GET["sfg"]) )
{
	...
	$sql = "SELECT * FROM category WHERE type LIKE '%animov%' AND genre LIKE '%$gen%'";
	$result = $dbconnect->query($sql);
	$NumberOfPosts = $result->num_rows;
	$NumberOfPages = ceil($NumberOfPosts / $PostsPerPage);
}
elseif( isset($_GET["sfc"]) AND isset($_GET["sfg"]))
{
	...
	$lang = $_GET["sfc"];
	$searchfilter = $_GET["sfc"];
	$sql = "SELECT * FROM category WHERE type LIKE '%animov%' AND language LIKE '%$lang%' AND genre LIKE '%$gen%' ";
	$result = $dbconnect->query($sql);
	$NumberOfPosts = $result->num_rows;
	$NumberOfPages = ceil($NumberOfPosts / $PostsPerPage);
}
...
else
{
	if ( !isset($_GET["sfc"]) )
	{
		$sql = "SELECT * FROM category WHERE type LIKE '%animov%'";
		$result = $dbconnect->query($sql);
		$NumberOfPosts = $result->num_rows;
		$NumberOfPages = ceil($NumberOfPosts / $PostsPerPage);
	}
...
<!DOCTYPE html>
<html>
<title>Q8Flix - Anime Movies</title>
<meta charset="UTF-8">
...
<div class="w3-row-padding w3-padding-16 w3-center">
	...
	if ( isset($_GET["sfm"]) OR isset($_GET["sfg"]) OR isset($_GET["sfc"]))
	{
		...
		<tr>
		<td colspan="2">
		<a href="animemovies.php"><div class="myButtonred w3-center" style="width:250px">Cancel Filters</div></a>
		</td>
		</tr>
		<?php
	}
	...
	<tr>
		...
		<option value="animemovies.php?sfm=mwatched<?php
		if ( isset($lang) )
		{
			echo "&sfc=".$lang;
		}
		?>" >Most Watched</option>
        <option value="animemovies.php?sfm=imdb<?php
		if ( isset($lang) )
		{
			echo "&sfc=".$lang;
		}
		?>" >IMDb Rating</option>
		<option value="animemovies.php?sfm=year<?php
		if ( isset($lang) )
		{
			echo "&sfc=".$lang;
		}
		?>" >Release Year</option>
		<option value="animemovies.php?sfm=name<?php
		if ( isset($lang) )
		{
			echo "&sfc=".$lang;
		}
		...
		<?php
			...
			{
        ?>
		<option value="animemovies.php?sfg=<?php 
		echo $allgenres[$i];
		if (isset($lang))
		{
			echo "&sfc=".$lang;
		...
		<?php
			...
			{
		?>
		<option value="animemovies.php?sfc=<?php 
		echo $alllanguages[$i];
		if (isset($gen))
		{
			echo "&sfg=".$gen;
...
if ( isset($_GET["sfc"]) AND in_array($lang,$alllanguages) AND !isset($_GET["sfg"]) AND !isset($_GET["sfm"]))
{
	...
	JOIN category AS c 
	ON c.id = p.catid 
	WHERE c.language LIKE '%$lang%' AND c.type LIKE '%animov%' 
	ORDER BY p.id DESC
...
elseif ( isset($_GET["sfg"]) AND in_array($gen,$allgenres) AND !isset($_GET["sfc"]))
{
	...
	JOIN posts AS p 
	ON c.id = p.catid 
	WHERE c.genre LIKE '%$gen%' AND c.type LIKE '%animov%' 
	ORDER BY p.id DESC 
	LIMIT $currentPage,$PostsPerPage
	";
	$result = $dbconnect->query($sql);
...
elseif (isset($_GET["sfc"]) AND in_array($lang,$alllanguages) AND isset($_GET["sfg"]) AND in_array($gen,$allgenres))
{
	...
	JOIN posts AS p 
	ON c.id = p.catid 
	WHERE c.genre LIKE '%$gen%' AND c.type LIKE '%animov%' AND c.language LIKE '%$lang%'
	ORDER BY p.id DESC 
	LIMIT $currentPage,$PostsPerPage
	";
	$result = $dbconnect->query($sql);
...
elseif( isset ( $_GET["sfm"]) AND $_GET["sfm"] == "mwatched" AND isset($lang)) 
{
	...
	$sql = "SELECT * , SUM(views) as tview
			FROM `posts` 
			WHERE 
			`catid` IN ( SELECT `id` FROM `category` WHERE `type` LIKE '%animov%') 
			AND
...
{
	$sql = "
	SELECT DISTINCT  c.id as catid, c.poster, c.title, c.imdbrating, c.releasedate, c.title as category
	FROM category AS c 
	WHERE c.type LIKE '%animov%'
	ORDER BY c.id DESC 
	LIMIT $currentPage,$PostsPerPage
...
?>
	...
	<?php
		 ...
		 if ( $page == 1 && $page < $NumberOfPages )
		 {
			 ...
			 echo 'animemovies.php?page='.$page; 
