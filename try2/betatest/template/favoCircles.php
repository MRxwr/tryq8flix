...
<table width="100%"><tr><td style="text-align: left; font-size: 20px;">Favorites</td></tr></table></div></b>
    <div id="jssor_6" style="position:relative;margin:0 auto;top:0px;left:0px;width:1000px;height:170px;overflow:hidden;visibility:hidden;">
        <!-- Loading Screen -->
...
$favoCats = selectDB("favourites","`userId` = '{$userID}' GROUP BY `categoryId` LIMIT 10");
for( $i = 0; $i < sizeof($favoCats); $i++ ){
	$listOfFavo = selectDB("posts","`catid` = '{$favoCats[$i]["categoryId"]}' LIMIT 1");
	?>
	<div>
		<a target="" href="category.php?id=<?php echo $listOfFavo[0]["catid"] ?>"><img data-u="image" src="<?php echo $listOfFavo[0]["poster"] ?>" style="border-radius: 50%;border-style:solid;border-width:2px;border-color:darkgrey" /></a>
	</div>
	<?php
}
?>
