...
$sql = "SELECT language FROM category WHERE type like '%movie%'";
...
$sql = "SELECT genre FROM category WHERE type like '%movie%'";
$result = $dbconnect->query($sql);
$NumberOfPosts = $result->num_rows;
$genre = array();
$allgenres = array();
...
elseif( isset($_GET["sfc"]) AND isset($_GET["sfg"]))
{
	...
	$lang = $_GET["sfc"];
	$searchfilter = $_GET["sfc"];
	$sql = "SELECT * FROM category WHERE type LIKE '%movie%' AND language LIKE '%$lang%' AND genre LIKE '%$gen%' ";
	$result = $dbconnect->query($sql);
	$NumberOfPosts = $result->num_rows;
	$NumberOfPages = ceil($NumberOfPosts / $PostsPerPage);
}
...
<div class="w3-row-padding w3-padding-16 w3-center">
	...
	if ( isset($_GET["sfm"]) OR isset($_GET["sfg"]) OR isset($_GET["sfc"]))
	{
		...
		<tr>
		<td colspan="2">
		<a href="newmovies.php"><div class="myButtonred w3-center" style="width:250px">Cancel Filters</div></a>
		</td>
		</tr>
		<?php
	}
	...
	<tr>
		...
		<option value="newmovies.php?sfm=mwatched<?php
		if ( isset($lang) )
		{
			echo "&sfc=".$lang;
		}
		?>" >Most Watched</option>
        <option value="newmovies.php?sfm=imdb<?php
		if ( isset($lang) )
		{
			echo "&sfc=".$lang;
		}
		?>" >IMDb Rating</option>
		<option value="newmovies.php?sfm=year<?php
		if ( isset($lang) )
		{
			echo "&sfc=".$lang;
		}
		?>" >Release Year</option>
		<option value="newmovies.php?sfm=name<?php
		if ( isset($lang) )
		{
			echo "&sfc=".$lang;
		}
		...
		<?php
			...
			{
        ?>
		<option value="newmovies.php?sfg=<?php 
		echo $allgenres[$i];
		if (isset($lang))
		{
			echo "&sfc=".$lang;
		...
		<?php
			...
			{
		?>
		<option value="newmovies.php?sfc=<?php 
		echo $alllanguages[$i];
		if (isset($gen))
		{
			echo "&sfg=".$gen;
...
elseif (isset($_GET["sfc"]) AND in_array($lang,$alllanguages) AND isset($_GET["sfg"]) AND in_array($gen,$allgenres))
{
	...
	JOIN posts AS p 
	ON c.id = p.catid 
	WHERE c.genre LIKE '%$gen%' AND c.type LIKE '%movie%' AND c.language LIKE '%$lang%'
	ORDER BY p.id DESC 
	LIMIT $currentPage,$PostsPerPage
	";
	$result = $dbconnect->query($sql);
...
elseif( isset ( $_GET["sfm"]) AND $_GET["sfm"] == "mwatched" AND isset($lang)) 
{
	...
	$sql = "SELECT * , SUM(views) as tview
			FROM `posts` 
			WHERE 
			`catid` IN ( SELECT `id` FROM `category` WHERE `type` LIKE '%movie%') 
			AND
