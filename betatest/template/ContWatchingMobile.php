...
<table width="100%"><tr><td style="text-align: left; font-size: 15px;color:white">Continue Watching</td></tr></table></div></b>
    <div id="jssor_7" style="position:relative;margin:0 auto;top:0px;left:0px;width:1000px;height:300px;overflow:hidden;visibility:hidden;">
        <!-- Loading Screen -->
...
$sql = "SELECT `videoId`
		...
		userId LIKE '".$userID."'
		AND
		`videoId` IN ( SELECT `id` FROM `posts` )
		GROUP BY `categoryId`
		ORDER BY id DESC
		LIMIT 10
		";
...
for ( $i = 0 ; $i < sizeof($ids) ; $i++ ){
	...
	?>
			...
			<div>
                <a target="" href="../watch.php?catid=<?php echo $row["catid"] ?>&postid=<?php echo $row["id"] ?>"><img data-u="image" src="<?php echo $row["poster"] ?>" style="border-radius: 10px;border-style:solid;border-width:1px;border-color:lightgray" /></a>
            </div>
